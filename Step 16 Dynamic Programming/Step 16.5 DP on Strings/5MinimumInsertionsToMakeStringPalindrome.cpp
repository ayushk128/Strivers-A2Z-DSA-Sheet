...
int LongestPalindromicSubsequence(int i, int j, string &s1, string &s2)
{
    ...
    if (s1[i] == s2[j])
        return 1 + LongestPalindromicSubsequence(i - 1, j - 1, s1, s2);
...
int LongestPalindromicSubsequenceMemoization(int i, int j, string &s1, string &s2, vector<vector<int>> &dp)
{
    ...
    if (s1[i] == s2[j])
        return dp[i][j] = 1 + LongestPalindromicSubsequenceMemoization(i - 1, j - 1, s1, s2, dp);
...
int LongestPalindromicSubsequenceMemoizationShiftedIndex(int i, int j, string &s1, string &s2, vector<vector<int>> &dp)
{
    ...
    if (s1[i - 1] == s2[j - 1])
        return dp[i][j] = 1 + LongestPalindromicSubsequenceMemoizationShiftedIndex(i - 1, j - 1, s1, s2, dp);
...
int minumumInsertions(string &s)
{
    ...
    // int lengthLPS = LongestPalindromicSubsequence(n - 1, n - 1, s, t);
    // return n - lengthLPS;

    // vector<vector<int>> dp(n, vector<int>(n, -1));
    // int lengthLPS =  LongestPalindromicSubsequenceMemoization(n - 1, n - 1, s, t, dp);
    // return n - lengthLPS;

    vector<vector<int>> dp(n + 1, vector<int>(n + 1, -1));
    int lengthLPS = LongestPalindromicSubsequenceMemoizationShiftedIndex(n, n, s, t, dp);
    return n - lengthLPS;
